    ...
    for x in range(min(k[0] for k in grid.keys()) - 1, max(k[0] for k in grid.keys()) + 2):
        for y in range(min(k[1] for k in grid.keys()) - 1, max(k[1] for k in grid.keys()) + 2):
            for z in range(min(k[2] for k in grid.keys()) - 1, max(k[2] for k in grid.keys()) + 2):
                for w in range(min(k[2] for k in grid.keys()) - 1, max(k[2] for k in grid.keys()) + 2):
                    current_activity = grid.get((x, y, z, w), False)
                        ...
                        for dy in (-1, 0, 1):
                            for dz in (-1, 0, 1):
                                for dw in (-1, 0, 1):
                                    if dx == dy == dz == dw == 0:
                                        continue
                                    elif grid.get((x + dx, y + dy, z + dz, w + dw), False):
